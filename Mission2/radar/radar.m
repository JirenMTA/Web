function radar(waveform, target, fc, Nchirp, Nsample)
    ...
    RDdB_padded = padarray(RDdB, [padR padD], 'symmetric', 'both');
    ...
    [detFlags, th] = cfar2d(RDdB_padded, cutidx);
    ...
    if isempty(detCells)
        ...
        [~, idx] = max(RDdB_padded(:));
        [ir, iv] = ind2sub(size(RDdB_padded), idx);
    else
        linIdx = sub2ind(size(RDdB_padded), detCells(1,:), detCells(2,:));
        [~, kbest] = max(RDdB_padded(linIdx));
        ir = detCells(1,kbest);
        iv = detCells(2,kbest);
    end
